                            ...
                            if ($pwd == $rePwd) {
                                ...
                                if ($select) {
                                    if (mysqli_num_rows($select) == 0) {
                                        ...
                                        if ($select) {
                                            if (mysqli_num_rows($select) == 0) {
                                                $req_3 = "INSERT INTO users (privilege,prenom,nom,pseudo,email,password,anniversaire,sexe,date_inscription,pp) VALUES (0,'$name','$lastName','$pseudo','$email','$pwd','$yy-$mm-$dd','$sex',now(),'pp.png')";

                                                $select = mysqli_query($connexion, $req_3);

                                                if ($select) {
                                                    header('Location:inscription.php?reg_err=success'); //Un probleme avec la synthaxe
                                                } else {
